...
bool EquipAction::UseEquipmentSet(string& name)
{
    ...
    {
        if (i->second.State == EQUIPMENT_SET_DELETED || i->second.Data.SetName != name)
            continue;
...
bool EquipAction::UseEquipmentSet(EquipmentSetInfo& set)
{
    ...
    {
        ObjectGuid guid;
        uint32 itemId = set.Data.Pieces[slot];
        if (set.Data.IgnoreMask & (1 << slot))
...
void EquipAction::TellEquipmentSets()
{
    ...
    for (EquipmentSetContainer::iterator i = sets.begin(); i != sets.end(); i++)
    {
        if (i->second.State != EQUIPMENT_SET_DELETED)
            ai->TellMaster(i->second.Data.SetName);
    }
}
